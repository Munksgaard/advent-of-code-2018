...
fun claimIsIntact arr claim =
    let val fields = claimToFields claim
    in List.all (fn (x, y) => Array2.sub (arr, x, y) = 1) fields
    end
...
fun solve input =
    ...
    in List.find (claimIsIntact arr) claims
    end

val () =
    let val input = TextIO.inputAll (TextIO.stdIn)
    in println (getOpt (Option.map (Int.toString o #id) (solve input),
                        "N/A"))
    end;
