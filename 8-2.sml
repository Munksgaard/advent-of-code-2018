...
fun childAtIndex xs i = SOME (List.nth (xs, i - 1)) handle Subscript => NONE

fun getValue (Node ([], metadata)) = foldl op+ 0 metadata
  | getValue (Node (childNodes, metadata)) =
    foldl op+ 0 (List.mapPartial (Option.map getValue o childAtIndex childNodes) metadata)

val () =
    let val input = TextIO.inputAll (TextIO.stdIn)
    in println (Int.toString (getValue (parse input)))
    end;
